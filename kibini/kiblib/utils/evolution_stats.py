...
class EvolutionActivite:
    ...
    def evolution_byweek_thisyear(self):
        stats_year = self.df[self.df['annee']==year]
        stat_year = stats_year.groupby('semaine')[self.df.columns[2]].sum().reset_index()
        return(stat_year)
        
    
    def evolution_hebdo_resume(self):
        stats_year_resume = self.df[self.df['annee']==year]
        stats_year_resume = stats_year_resume.groupby('semaine')[self.df.columns[2]].sum().reset_index()
        mean_year = np.int64(round(stats_year_resume.iloc[:,1].mean(),0))
        sum_year = stats_year_resume.iloc[:,1].sum()
        return(f"Depuis le debut de l annee, on compte au total {sum_year} {self.text} avec en moyenne {mean_year} {self.text} par semaine")
        #return(stat_year_resume)
    
    def evolution_4lastweek(self):
        semaine_max = date.today().isocalendar()[1] - 1
        semaine_min = semaine_max - 4
        stats_4lastweek = self.df[self.df['annee'].isin([2023,2024,year])]
    ...
    def evolution_by_year(self):
        #stats_year = self.df[self.df['annee'].isin([2023,2024,year])]
        stats_year = self.df.groupby('annee')[self.df.columns[2]].sum().reset_index()
        return(stats_year)
        
    def distinct_evolution_byweek_thisyear(self):
        distinct_stats_year = self.df[self.df['annee']==year]
        distinct_stats_year = distinct_stats_year.groupby('semaine')[self.df.columns[1]].nunique().reset_index()
    ...
    def distinct_evolution_4lastweek(self):
        semaine_max = date.today().isocalendar()[1] - 1
        semaine_min = semaine_max - 4
        distinct_stats_4lastweek =  self.df[self.df['annee'].isin([2023,2024,year])]
    ...
    def distinct_evolution_by_year(self):
        #distinct_stats_year =  self.df[self.df['annee'].isin([2023,2024,year])]
    ...
    def titre_graph1(self):
        return(f"Evolution du nombre de {self.text} par semaine en {year}")

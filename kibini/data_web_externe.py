...
# Import du fichier
stat_web_externe = pd.read_csv("/home/kibini/stat_web_externe.csv",encoding="utf-16")
...
web_externe_to_import = stat_web_externe[colonnes_to_keep]
web_externe_to_import.head(10)

#creation d'un dictionnaire de correspondance des colonnes PIWIK et STATDB
...
web_externe_to_import = web_externe_to_import.rename(columns=dico_stats_web)
...
web_externe_to_import['taux_conversion'] = web_externe_to_import['taux_conversion'].apply(extraire_chiffres)


# On arrondi à l'unité la valeur dans TAUX_CONVERSION, puis on transforme en INT
web_externe_to_import['taux_conversion'] = round(web_externe_to_import['taux_conversion'],0).astype(int)
...
# Ajout d'une colonne origine avec la valeur externe
web_externe_to_import['origine'] = 'externe'

#Jointure gauche des deux tables
web_externe_to_import = web_externe_to_import.merge(verif_vacances,left_on='date',right_on='date',how='left')

# Import des données dans statdb
web_externe_to_import.to_sql('stat_web2',con=db_conn,if_exists='append',index=False)

# Affichage du résultat
print(web_externe_to_import)
...
def get_items(annee, location, db_conn, c2lDict, indices):
    ...
    # COUNT(IF(i.dateaccessioned > ('2023-01-01' - INTERVAL 1 YEAR), i.itemnumber, NULL)) AS 'nb_exemplaires_créés_dans_annee'               
    ...
    #COUNT(IF(i.notforloan NOT IN (-2, -1, 2) AND i.datelastborrowed <= ('2023-01-01' - INTERVAL 1 YEAR), i.itemnumber, NULL)) AS 'nb_exemplaires_empruntables_pas_empruntés_1_an',
    ...
    #COUNT(IF(i.notforloan NOT IN (-2, -1, 2) AND i.datelastborrowed <= ('2023-01-01' - INTERVAL 3 YEAR), i.itemnumber, NULL)) AS 'nb_exemplaires_empruntables_pas_empruntés_3_ans',
...
def get_issues(annee, location, db_conn, c2lDict, indices):
    ...
    for idx in prets_pv.index:
        ...
        if idx[4] != '':
            ...
            e2022 = p['borrowernumber'][(p['pret_date_pret_annee'] == '2022')
                                      ...
                                      & (p['doc_item_collection_lib4'] == idx[3])
                                      & (p['doc_biblio_support'] == idx[4])].nunique()
            e2021 = p['borrowernumber'][(p['pret_date_pret_annee'] == '2021')
                                      & (p['doc_item_collection_lib1'] == idx[0])
                                      & (p['doc_item_collection_lib2'] == idx[1])
                                      & (p['doc_item_collection_lib3'] == idx[2])
                                      & (p['doc_item_collection_lib4'] == idx[3])
            ...
            ex2022 = p['itemnumber'][(p['pret_date_pret_annee'] == '2022')
                                      ...
                                      & (p['doc_item_collection_lib4'] == idx[3])
                                      & (p['doc_biblio_support'] == idx[4])].nunique()
            ex2021 = p['itemnumber'][(p['pret_date_pret_annee'] == '2021')
                                      & (p['doc_item_collection_lib1'] == idx[0])
                                      & (p['doc_item_collection_lib2'] == idx[1])
                                      & (p['doc_item_collection_lib3'] == idx[2])
                                      & (p['doc_item_collection_lib4'] == idx[3])
            ...
            e2021 = p['borrowernumber'][(p['pret_date_pret_annee'] == '2021')].nunique()
            ex2023 = p['itemnumber'][(p['pret_date_pret_annee'] == '2023')].nunique()
            ex2022 = p['itemnumber'][(p['pret_date_pret_annee'] == '2022')].nunique()
            ex2021 = p['itemnumber'][(p['pret_date_pret_annee'] == '2021')].nunique()
                                      
        idxx.append(idx)
        emprunteurs_2023.append(e2023)
        emprunteurs_2022.append(e2022)
        emprunteurs_2021.append(e2021)
        exemplaires_2023.append(ex2023)
        exemplaires_2022.append(ex2022)
        exemplaires_2021.append(ex2021)
    
    prets_pv['nb_prets_exemplaires_distincts_2021'] = exemplaires_2021
    ...
    prets_pv['nb_emprunteurs_distincts_2021'] = emprunteurs_2021
...
def get_eliminations(annee, location, db_conn, c2lDict, indices):
    ...
    WHERE i.annee_mise_pilon = '2023' AND {location}"""
...
writer = pd.ExcelWriter('../data/Statistiques_collections_2023.xlsx', engine='xlsxwriter')
...
for site in sites:
    ...
    stats.to_csv(f"../data/Statistiques_collections_2023_TMP2_{site['code']}.csv")
    
    print("---- tx")
    stats['evolution_prets_2021-2022'] = (stats['nb_prets_2022'] - stats['nb_prets_2021']) / stats['nb_prets_2021']
    stats['evolution_prets_2021-2023'] = (stats['nb_prets_2023'] - stats['nb_prets_2021']) / stats['nb_prets_2021']
    stats['evolution_prets_2022-2023'] = (stats['nb_prets_2023'] - stats['nb_prets_2022']) / stats['nb_prets_2022']
    
    stats['evolution_prets_exemplaires_distincts_2021-2022'] = (stats['nb_prets_exemplaires_distincts_2022'] - stats['nb_prets_exemplaires_distincts_2021']) / stats['nb_prets_exemplaires_distincts_2021']
    stats['evolution_prets_exemplaires_distincts_2021-2023'] = (stats['nb_prets_exemplaires_distincts_2023'] - stats['nb_prets_exemplaires_distincts_2021']) / stats['nb_prets_exemplaires_distincts_2021']
    ...
    stats['tx_rotation'] = round( stats['nb_prets_2023'] / stats['nb_exemplaires_empruntables'], 1)
    stats['tx_fond_actif'] =  stats['nb_prets_exemplaires_distincts_2023'] / stats['nb_exemplaires_empruntables']
    ...
    stats['evolution_emprunteurs_distincts_2021-2022'] = (stats['nb_emprunteurs_distincts_2022'] - stats['nb_emprunteurs_distincts_2021']) / stats['nb_emprunteurs_distincts_2021']
    stats['evolution_emprunteurs_distincts_2021-2023'] = (stats['nb_emprunteurs_distincts_2023'] - stats['nb_emprunteurs_distincts_2021']) / stats['nb_emprunteurs_distincts_2021']
    ...
    stats = stats[['nb_exemplaires', 'nb_exemplaires_empruntables', 'exemplaires_consultables_sur_place',
                   ...
                   'nb_exemplaires_elimines_abimes', 'nb_exemplaires_elimines_desherbes', 'nb_prets_2021',
                   'nb_prets_exemplaires_distincts_2021', 'nb_prets_2022', 'nb_prets_exemplaires_distincts_2022', 'nb_prets_2023',
                   'nb_prets_exemplaires_distincts_2023', 'nb_exemplaires_empruntables_pas_empruntés_1_an',
                   'nb_exemplaires_empruntables_pas_empruntés_3_an', 'nb_exemplaires_en_pret', 'evolution_prets_2021-2022',
                   'evolution_prets_2021-2023', 'evolution_prets_2022-2023', 'evolution_prets_exemplaires_distincts_2021-2022',
                   'evolution_prets_exemplaires_distincts_2021-2023', 'evolution_prets_exemplaires_distincts_2022-2023', 'tx_rotation',
                   'tx_fond_actif', 'tx_collection_non_empruntee_3_ans', 'tx_sortie', 'tx_non_restitues', 'tx_renouvellement',
                   'tx_accroissement', 'nb_emprunteurs_distincts_2021', 'nb_emprunteurs_distincts_2022', 'nb_emprunteurs_distincts_2023',
                   'evolution_emprunteurs_distincts_2021-2022', 'evolution_emprunteurs_distincts_2021-2023',
                   'evolution_emprunteurs_distincts_2022-2023']]
    ...
    stats.columns = ["exemplaires", "exemplaires empruntables", "exemplaires consultables sur place uniquement", "exemplaires en accès libre",
                     ...
                     "exemplaires éliminés non restitués", "exemplaires éliminés perdus", "exemplaires éliminés abîmés", "exemplaires éliminés désherbés", "prêts 2021",
                     "prêts 2021 exemplaires distincts", "prêts 2022", "prêts 2022 exemplaires distincts", "prêts 2023", "prêts 2023 exemplaires distincts",
                     "exemplaires empruntables pas empruntés 1 an", "exemplaires empruntables pas empruntés 3 ans", "exemplaires en prêt", "évolution prêts 2021-2022",
                     "évolution prêts 2021-2023", "évolution prêts 2022-2023", "évolution exemplaires distincts prêtés 2021-2022", "évolution exemplaires distincts prêtés 2021-2023",
                     "évolution exemplaires distincts prêtés 2022-2023", "taux de rotation", "taux de fonds actif", "part collection non empruntée depuis 3 ans", "taux de sortie",
                     "taux de non restitués", "taux renouvellement", "taux d'accroissement", "emprunteurs distincts 2021", "emprunteurs distincts 2022", "emprunteurs distincts 2023",
                     "évolution emprunteurs distincts 2021-2022", "évolution emprunteurs distincts 2021-2023", "évolution emprunteurs distincts 2022-2023"]

...
**Bug trouvé et corrigé au premier essai** : `SUM(entrees)`/`SUM(nb_pages_imprimees)` peuvent ressortir à `0` explicitement (l'API Opteio distingue parfois entrées et sorties sur deux lignes séparées - un événement "sortie seule" crée une ligne `stat_entrees` avec `entrees=0` ; de même un travail d'impression à 0 page). Contrairement à `COUNT(*)` (toujours ≥ 1 par construction du `GROUP BY`), ça introduisait une ligne fantôme dans la jointure externe quand rien d'autre n'avait d'activité ce créneau-là - confirmé sur 38 lignes entièrement à zéro (2022 et 2026, surtout 18h-19h) lors du premier essai avec `entrees`. Corrigé par `HAVING SUM(...) > 0` sur ces deux requêtes (`stat_affluence2oa.py` et `stat_affluence_concat.py`), plus un filet de sécurité qui retire toute ligne encore entièrement nulle sur les 7 métriques (`occupation` exclue, voir plus bas).
...
**Comportement à connaître, pas un bug** : `occupation` est fusionnée en jointure **gauche** sur le fichier déjà construit à partir des 7 autres métriques - elle ne crée jamais de nouvelle ligne. La correction ramène mathématiquement toujours l'occupation à exactement 0 au dernier créneau *réellement mesuré par le capteur* ce jour-là (vérifié sur un cas simulé) - mais si le capteur de porte continue d'enregistrer des mouvements après que prêts/retours/wifi/postes/impressions/salle d'étude se sont arrêtés pour la journée (ex. personnel repartant après la fermeture au public), ce créneau de fermeture n'apparaît dans aucune des 7 autres métriques et **n'apparaît pas dans le fichier final**. Constaté sur environ 13% des jours (532/4098, mesuré avant l'ajout de `frequentation_etude` et l'exclusion ponctuelle du 2026-03-30 - ordre de grandeur, pas une valeur figée) : le dernier créneau *visible* dans le fichier n'y montre alors pas `occupation=0`, sans que ce soit une erreur de calcul - juste le vrai zéro de fermeture qui tombe sur un créneau non publié. Choix assumé : ne pas passer `occupation` en jointure externe pour éviter d'ajouter des lignes où les 7 autres métriques seraient artificiellement à 0.
